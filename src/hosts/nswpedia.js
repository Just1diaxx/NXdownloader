...
class NSWPedia extends BaseHost {
    constructor() {
        super();
        this.name = "NSWPedia";
        this.baseUrl = "https://nswpedia.com";
    }
    ...
    async getDownloadBlocks(gameUrl) {
        ...
        const blocks = await this.getRealBlocks(href);

        return blocks;
    }

    async getRealBlocks(url) {
        ...
        for (const row of rows) {
            ...
            const downloadPageUrl = new URL(a.attr("href"), this.baseUrl).href;

            const realUrl = await this.getRealUrl(downloadPageUrl, page);
            const host = this.getDomainName(realUrl);
    ...
    async getRealUrl(url, page) {
        await page.goto(url, { waitUntil: 'domcontentloaded' });

        await page.waitForSelector('#download-link', { timeout: 6000 });
